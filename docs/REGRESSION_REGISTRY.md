...
| 13 | **Portadas de artistas sugeridos no cargan** | `resize()` trunca los avatares `yt3.ggpht.com/…=s…` (`split("-s")[0]`) → 404. Álbumes usan `lh3…=w…` (otra rama, ok) | `Items.kt` ItemThumbnail param `resize`; call sites pasan `resize = item !is ArtistItem` | 0.6.95 | ✅ |
